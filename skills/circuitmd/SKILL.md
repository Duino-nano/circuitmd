...
- **1行に書ける部品は1つ**。`NMOS Q1 ↓ GND` のように部品名を並べても端子は生えず、
  2つ目はただのラベル文字になる → `GND @Q1.source` と行を分ける
- **接続**: `@Q1.base` `@(2,1.5)`（座標はスペースなし）。アンカーはBJTが `.base/.collector/.emitter`、
  FETが `.gate/.drain/.source`、2端子素子が `.start/.end`
- **`@` は素子を置く「起点」で、1行に1つだけ**（2つ書くと後勝ちで素子が飛ぶ＝エラー）
- **この記法は「つながり」ではなく「図形の並べ方」を書く**。ネットリストのように
  「AとBをつなぐ」と書いても配置は計算されない。素子は 方向＋`len=` で置き、
  離れた点へは `線` と `tox=`/`toy=` で直交させて届かせる。
  `to=@終点` は始点と終点のXかYが揃っているときだけ使える（揃っていないと素子が
  斜めになり**エラー**）。**方向と `to=` の併用は禁止**（矛盾するのでエラー。
  「向きと長さで置く」なら方向＋`len=`、「決まった2点に渡す」なら `to=` 単独）
- **`# allow-diagonal` はエラー回避に使わない**。ブリッジ整流のひし形など、斜めが
  設計上正しいときだけの逃げ道で、書いても「斜め素子N個を許容しました」と毎回
  報告される。図が崩れているのを黙らせる目的で使うと、崩れたまま納品してしまう
- **多端子素子（NPN/PNP/NMOS/PMOS/オペアンプ）に方向を付けない**。方向は素子の姿勢を
  回転させるだけで接続方向は変わらない。姿勢を変えるなら `rev`（左右反転）/`flip`（上下反転）
...
- **オプション**: `loc=下`（ラベル位置） `len=1.5`（長さ） `tox=@X.end` `toy=@Y.end`（座標合わせ）
  `to=@X.end`（終点まで渡す） `at=@X.end`（`@X.end` と同じ） `ofst=0.4`（ラベルを線から離す）
  `rev`（左右反転） `flip`（上下反転）。**これ以外の `key=値` はラベル文字として描かれる**（警告が出る）
...
- **`.at()` は `d.add()` の内側に書く**。`d.add(elm.Ic(...)).at((3,3))` は**黙って無視され**、
  素子は直前のカーソル位置に取り残される（エラーが出ないので気づけない）。
  配置は `d.add()` した時点で確定する
